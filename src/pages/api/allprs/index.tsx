...
const EipPrDetails =
  mongoose.models.alleipsprdetails ||
  mongoose.model('alleipsprdetails', prDetailsSchema);
const ErcPrDetails =
  mongoose.models.allercsprdetails ||
  mongoose.model('allercsprdetails', prDetailsSchema);
const RipPrDetails =
  mongoose.models.allripsprdetails ||
  mongoose.model('allripsprdetails', prDetailsSchema);

const getAllPrs = async (req: Request, res: Response) => {
  try {
    // Retrieve unique PR numbers from each collection with repository information
    const eipPrNumbers = await EipPrDetails.find(
      {},
      { prTitle: 1, prNumber: 1, _id: 0 }
    ).lean();
