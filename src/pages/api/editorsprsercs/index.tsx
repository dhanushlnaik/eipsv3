...
const handler = async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        const githubHandles = [
            'CarlBeek', 'nconsigny', 'yoavw', 'adietrichs', 'axic', 'gcolvin', 'lightclient',
            'SamWilsn', 'xinbenlv', 'g11tech', 'cdetrio', 'Pandapip1', 'Souptacular',
            'wanderer', 'MicahZoltu', 'arachnid', 'nicksavers', 'vbuterin',
        ];

        // Initialize the result object with empty arrays for each reviewer
        const resultByReviewer = githubHandles.reduce((acc, handle) => {
            acc[handle] = [];
            return acc;
