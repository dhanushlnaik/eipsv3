...
const getPRReviewDetails = async (req: Request, res: Response): Promise<void> => {
    try {
        // GitHub handles to filter by
        const githubHandles = [
            'CarlBeek', 'nconsigny', 'yoavw', 'adietrichs', 'axic', 'gcolvin', 
            'lightclient', 'SamWilsn', 'xinbenlv', 'g11tech', 'cdetrio', 'Pandapip1', 
            'Souptacular', 'wanderer', 'MicahZoltu', 'arachnid', 'nicksavers', 'vbuterin'
        ];

        // Create an object to store the results for each reviewer
        const resultByReviewer: { [key: string]: { repo: string; prNumber: number; prTitle: string; created_at: Date; closed_at: Date; merged_at: Date; reviewDate: Date; reviewComment: string }[] } = {};
        ...
        eipReviews.forEach((review) => {
            const {
                reviewerName, prNumber, prTitle, created_at, closed_at, merged_at, reviewDate, reviewComment,
            } = review;
