...
const EIPReviewDetails = mongoose.models.EIPReviewDetails || mongoose.model('EIPReviewDetails', eipReviewSchema);

// Controller to fetch and group PR review details by reviewer
const fetchAndGroupPRDetails = async (req: Request, res: Response): Promise<void> => {
    try {
        // Fetch GitHub reviewer handles
        const githubHandles = await fetchReviewers();

        if (!githubHandles || githubHandles.length === 0) {
            res.status(404).json({ error: 'No reviewers found from GitHub configuration' });
            return;
        }
