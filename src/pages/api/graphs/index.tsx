...
const handler = async (req: Request, res: Response) => {
    StatusChange.aggregate([
        ...
        .then((result: { _id: string, eips: { category: string, changedYear: number, changedMonth: number, count: number, eips: typeof StatusChange[] }[] }[]) => {
            const formattedResult = result.map((group: { _id: string, eips: { category: string, changedYear: number, changedMonth: number, count: number, eips: typeof StatusChange[] }[] }) => ({
                status: group._id,
                eips: group.eips.reduce((acc: { category: string, month: number, year: number, date: string, count: number }[], eipGroup: { category: string, changedYear: number, changedMonth: number, count: number, eips: typeof StatusChange[] }) => {
