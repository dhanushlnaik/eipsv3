...
const ERCTypeDonut = () => {
  ...
  const typeData = Array.from(new Set(data.map((item) => item.category)));
  ...
  const colors = [
    ...
    { main: 'rgba(77, 182, 172, 0.7)', stroke: '#4DB6AC' }, // Teal (NEW for extra category)
  ];

  const typeChartData = typeData.map((category) => ({
    name: category,
    value: data.filter((item) => item.category === category).length,
  }));

  const formattedTypeData = typeChartData.map((item, index) => ({
    ...item,
    itemStyle: {
  ...
  return (
    ...
    >
      ...
      <ReactECharts option={typeChartOption} style={{ height: '400px' }} />
    </motion.div>
  );
};

export default ERCTypeDonut;
