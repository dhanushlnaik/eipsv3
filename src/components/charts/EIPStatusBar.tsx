...
const EIPStatusBar: React.FC<ChartProps> = ({ type }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (type === 'EIP') setData(jsonData.eip);
        else if (type === 'ERC') setData(jsonData.erc);
        else if (type === 'RIP') {
          jsonData.rip.forEach((item: EIP) => {
            if (item.eip === '7859') item.status = 'Draft';
          });
          setData(jsonData.rip);
        } else setData([...jsonData.eip, ...jsonData.erc, ...jsonData.rip]);
        setIsLoading(false);
      } catch (error) {
  ...
  const seriesMap = transformedData.reduce((acc, { status, year, value }) => {
    if (!acc.has(status)) acc.set(status, []);
    acc.get(status)?.push({ year, value });
    return acc;
